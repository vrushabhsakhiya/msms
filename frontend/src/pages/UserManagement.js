    ...
    <Layout>
      ...
      {showUserModal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.55)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 3000, padding: "1rem" }}>
          <div className="card" style={{ width: "100%", maxWidth: "560px", borderRadius: "20px", padding: "2rem" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.5rem" }}>
              <h3 style={{ margin: 0, display: "flex", alignItems: "center", gap: "8px" }}>
                {editUserMode ? <Pencil size={20} style={{ color: "var(--primary)" }} /> : <UserPlus size={20} style={{ color: "var(--primary)" }} />}
                {editUserMode ? "Edit Staff Member" : "Add Staff Member"}
              </h3>
              <button onClick={() => setShowUserModal(false)} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--text-muted)" }}><X size={20} /></button>
            </div>

            <form onSubmit={handleCreateOrUpdateUser}>
              <div className="grid-cols-2">
                <div className="form-group" style={{ gridColumn: "1 / -1" }}>
                  <label style={{ fontWeight: "600", fontSize: "0.85rem", marginBottom: "4px" }}>Full Name *</label>
                  <input required placeholder="Min 3 characters, alphabets only" pattern="[A-Za-z ]{3,100}" className="custom-input" value={userForm.full_name} onChange={(e) => setUserForm({ ...userForm, full_name: e.target.value })} />
                </div>

                <div className="form-group">
                  <label style={{ fontWeight: "600", fontSize: "0.85rem", marginBottom: "4px" }}>Username *</label>
                  <input required disabled={editUserMode} placeholder="Unique login name" className="custom-input" value={userForm.username} onChange={(e) => setUserForm({ ...userForm, username: e.target.value })} />
                </div>

                <div className="form-group">
                  <label style={{ fontWeight: "600", fontSize: "0.85rem", marginBottom: "4px" }}>Email *</label>
                  <input required type="email" placeholder="Valid email" className="custom-input" value={userForm.email} onChange={(e) => {
                    const val = e.target.value;
                    const prefix = val.split('@')[0];
                    if (!editUserMode) {
                      setUserForm({ ...userForm, email: val, password: prefix ? `${prefix}#123` : "" });
                    } else {
                ...
                <div className="form-group">
                  <label style={{ fontWeight: "600", fontSize: "0.85rem", marginBottom: "4px" }}>Mobile *</label>
                  <input required placeholder="Exactly 10 digits" pattern="[6-9][0-9]{9}" maxLength={10} className="custom-input" value={userForm.mobile} onChange={(e) => setUserForm({ ...userForm, mobile: e.target.value.replace(/\D/g, "") })} />
                </div>

                <div className="form-group">
                  <label style={{ fontWeight: "600", fontSize: "0.85rem", marginBottom: "4px" }}>Role *</label>
                  <select required className="custom-input" value={userForm.role} onChange={(e) => setUserForm({ ...userForm, role: e.target.value })}>
              ...
              <div className="form-group" style={{ display: "flex", gap: "10px", alignItems: "center", marginTop: "0.5rem", marginBottom: "1rem" }}>
                <label style={{ fontWeight: "600", fontSize: "0.85rem", display: "flex", alignItems: "center", gap: "5px" }}>Status:</label>
                <button type="button" onClick={() => setUserForm({ ...userForm, is_active: !userForm.is_active })} style={{ background: userForm.is_active ? "#10b981" : "#e2e8f0", border: "none", borderRadius: "20px", width: "40px", height: "24px", position: "relative", cursor: "pointer", transition: "0.3s" }}>
                  <div style={{ width: "18px", height: "18px", background: "white", borderRadius: "50%", position: "absolute", top: "3px", left: userForm.is_active ? "19px" : "3px", transition: "0.3s" }} />
                </button>
                <span style={{ fontSize: "0.85rem", color: userForm.is_active ? "#10b981" : "#94a3b8", fontWeight: "700" }}>{userForm.is_active ? "Active" : "Inactive"}</span>
              </div>

              {!editUserMode && (
                <div className="form-group">
                  <label style={{ fontWeight: "600", fontSize: "0.85rem", marginBottom: "4px" }}>Initial Password (auto-generated)</label>
                  <input required type="text" className="custom-input" style={{ fontFamily: "monospace", background: "#f8fafc" }} value={userForm.password} onChange={(e) => setUserForm({ ...userForm, password: e.target.value })} />
                </div>
              )}

              <div style={{ display: "flex", gap: "0.75rem", marginTop: "1.5rem" }}>
                <button type="submit" className="btn-primary" style={{ flex: 1, borderRadius: "10px", display: "flex", justifyContent: "center" }}>
                  <Save size={16} /> {editUserMode ? "Save Changes" : "Save & Send Email"}
                </button>
                <button type="button" onClick={() => setShowUserModal(false)} style={{ flex: 0.5, borderRadius: "10px", border: "1px solid var(--border)", background: "white", color: "var(--text-muted)" }}>Cancel</button>
              </div>
            </form>
          </div>
        </div>
